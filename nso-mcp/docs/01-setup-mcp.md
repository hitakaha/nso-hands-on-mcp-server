...
このボでは、広範な **`permit`** ポリシーでNSO MCPを公開したままにしました。次のシナリオ [bgpmgr サービス](02-nso-mcp-services-bgp.md) では、**追加のサービスパッケージ**（`bgpmgr`）をロードし、そのツールがMCPクライアントに表示されるのを確認します。そして、自然言語を使用して **xr-1** と **xr-2** 間の **BGPを設定** し、2つ目のMCPクライアント（`ollmcp`）から利用可能なツールとプロンプトを検査します。
