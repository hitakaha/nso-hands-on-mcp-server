...
このセッションでは、**NSO 6.7**の目玉機能である**Model Context Protocol (MCP) サーバー**について、
...
![teratermi](assets/images/lab02/teraterm.png)
...
![同期された2つのXRdルータを示すデバイスのsync-fromを実行したNSO CLI](assets/images/lab02/nso-cli-sync-from-xrd.png)
...
![5つのテストすべてに合格したことを示すtest-mcp.shの出力](assets/images/lab02/test-mcp-all-tests-passed.png)
...
Ubuntu では Firefoxが自動的に開くはずです。
開かない場合または Win11 の場合は、手動でFirefoxを起動し、<http://198.18.134.27:8501/> にアクセスしてください。

![backend-frontend-ui](assets/images/lab02/backend-frontend-ui.png)


Web UIの左側のメニューに **NSO MCP tools** が表示されていることを確認します。

![左側のメニューにNSO MCPツールがリストされているStreamlit Web MCPクライアント](assets/images/lab02/mcp-web-client-tools-menu.png)
...
![NSOデバイスに関する自然言語の質問に答えるWeb MCPクライアント](assets/images/lab02/mcp-web-client-natural-language.png)
...
![ポリシーをpermitに変更した後、100個以上のツールを表示しているMCP Webクライアントの左メニュー](assets/images/lab02/mcp-policy-permit-100-tools.png)

これで、よりリッチなクエリを実行できるようになります。例えば、デバイスの **`live-status`** を表示するようにクライアントに依頼します。

![XRdデバイスのlive-status出力を返すMCP Webクライアント](assets/images/lab02/mcp-live-status-result.png)
...
このボでは、広範な **`permit`** ポリシーでNSO MCPを公開したままにしました。[bgpmgr サービス](02-nso-mcp-services-bgp.md) では、**追加のサービスパッケージ**（`bgpmgr`）をロードし、そのツールがMCPクライアントに表示されるのを確認します。そして、自然言語を使用して **xr-1** と **xr-2** 間の **BGPを設定** し、2つ目のMCPクライアント（`ollmcp`）から利用可能なツールとプロンプトを検査します。
